...
$sTableID = "davidok95_psk_destinations"; // ID таблицы
...
if (CheckFilter())
{
	...
	if ($find_price)
		$arFilter["CITY_RUS"] = $find_city_rus;
	if ($find_zone)
...
if($lAdmin->EditAction() && $POST_RIGHT=="W")
{
    ...
    foreach($FIELDS as $ID => $arFields)
    {
        ...
        $ID = IntVal($ID);
		$rsData = PskDestinationsTable::getList(array(
			'filter' => array("ID" => $ID),
		));
		if ($arData = $rsData->Fetch())
        {
			...
			PskDestinationsTable::update($ID, $arData);
        }
        else
...
$lAdmin->AddHeaders(array(
	...
	array(
		"id"       => "CITY",
		"content"  => Loc::getMessage("DAVIDOK95_PSK_DESTINATIONS_CITY"),
		"sort"     => "CITY",
		"default"  =>true,
	),
	array(
		"id"       => "REGION",
		"content"  => Loc::getMessage("DAVIDOK95_PSK_DESTINATIONS_REGION"),
		"sort"     => "REGION",
		"default"  =>true,
	),
	array(
		"id"       => "CITY_RUS",
		"content"  => Loc::getMessage("DAVIDOK95_PSK_DESTINATIONS_CITY_RUS"),
		"sort"     => "CITY_RUS",
		"default"  =>true,
	),
	array(
		"id"       => "ZONE",
		"content"  => Loc::getMessage("DAVIDOK95_PSK_DESTINATIONS_ZONE"),
		"sort"     => "ZONE",
		"default"  =>true,
	),
	array(
		"id"       => "ZIP_CODE",
		"content"  => Loc::getMessage("DAVIDOK95_PSK_DESTINATIONS_ZIP_CODE"),
		"sort"     => "ZIP_CODE",
		"default"  =>true,
	),
));
...
$oFilter = new CAdminFilter(
	...
	array(
		"ID",
		Loc::GetMessage("DAVIDOK95_PSK_DESTINATIONS_CITY"),
		Loc::GetMessage("DAVIDOK95_PSK_DESTINATIONS_REGION"),
		Loc::GetMessage("DAVIDOK95_PSK_DESTINATIONS_CITY_RUS"),
		Loc::GetMessage("DAVIDOK95_PSK_DESTINATIONS_ZONE"),
		Loc::GetMessage("DAVIDOK95_PSK_DESTINATIONS_ZIP_CODE"),
	)
);
?>
<form name="find_form" method="get" action="<?echo $APPLICATION->GetCurPage();?>">
	...
	<tr>
		<td><?= Loc::GetMessage("DAVIDOK95_PSK_DESTINATIONS_CITY") ?>:</td>
		<td>
			<input type="text" name="find_city" size="47" value="<?echo htmlspecialchars($find_city)?>">
		</td>
	</tr>
	<tr>
		<td><?= Loc::GetMessage("DAVIDOK95_PSK_DESTINATIONS_REGION") ?>:</td>
		<td>
			<input type="text" name="find_region" size="47" value="<?echo htmlspecialchars($find_region)?>">
		</td>
	</tr>
	<tr>
		<td><?= Loc::GetMessage("DAVIDOK95_PSK_DESTINATIONS_CITY_RUS") ?>:</td>
		<td>
			<input type="text" name="find_city_rus" size="47" value="<?echo htmlspecialchars($find_city_rus)?>">
		</td>
	</tr>
	<tr>
		<td><?= Loc::GetMessage("DAVIDOK95_PSK_DESTINATIONS_ZONE") ?>:</td>
		<td>
			<input type="text" name="find_zone" size="47" value="<?echo htmlspecialchars($find_zone)?>">
		</td>
	</tr>
	<tr>
		<td><?= Loc::GetMessage("DAVIDOK95_PSK_DESTINATIONS_ZIP_CODE") ?>:</td>
		<td>
			<input type="text" name="find_zip_code" size="47" value="<?echo htmlspecialchars($find_zip_code)?>">
		</td>
	</tr>
